...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validação
    if (empty($username) || empty($nome_completo) || empty($email) || empty($senha) || empty($genero) || empty($endereco) || empty($idade) || empty($modalidade)) {
    ...
    } elseif (!in_array($modalidade, ['Futebol','Voleibol','Basquetebol','Sem modalidade'])) {
        ...
        if ($result_check->num_rows > 0) {
            $error_message = "Nome de usuário ou e-mail já registrado.";
        } else {
            // Inserção segura
            $sql = "INSERT INTO usuarios (username, nome_completo, email, senha, tipo, genero, endereco, idade, modalidade) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            
            if (!$stmt) {
                $error_message = "Erro interno: " . $conn->error;
            } else {
                // 9 parâmetros
                $stmt->bind_param("sssssssis", $username, $nome_completo, $email, $hash, $tipo, $genero, $endereco, $idade, $modalidade);
...
<head>
    ...
    <style>
        ...
        .register-card h2 { text-align: center; margin-bottom: 30px; font-weight: 700; color: var(--primary); }
        .input-group-text { background-color: #eee; border: 1px solid #ced4da; border-right: none; }
        .input-group .form-select, .form-control { border-radius: 0 8px 8px 0 !important; padding: 10px 15px; font-size: 1rem; }
        .btn-primary { background-color: var(--primary); border: none; padding: 12px; font-size: 1rem; font-weight: 600; border-radius: 8px; transition: background-color 0.3s ease; }
        .btn-primary:hover { background-color: #0056b3; }
        .back-link { display: block; text-align: center; margin-top: 20px; color: var(--secondary); text-decoration: none; transition: color 0.3s ease; }
...
<div class="register-card">
    ...
    <form action="register.php" method="post">
        ...
        <div class="mb-3">
            <label class="form-label visually-hidden">Nome Completo</label>
            <div class="input-group">
                <span class="input-group-text"><i class="fa-solid fa-id-card"></i></span>
                <input type="text" name="nome_completo" class="form-control" placeholder="Nome Completo" required value="<?= htmlspecialchars($_POST['nome_completo'] ?? '') ?>">
            </div>
        </div>

        <div class="mb-3">
            <label class="form-label visually-hidden">Idade</label>
            <div class="input-group">
                <span class="input-group-text"><i class="fa-solid fa-calendar"></i></span>
                <input type="number" name="idade" class="form-control" placeholder="Idade" required value="<?= htmlspecialchars($_POST['idade'] ?? '') ?>" min="1" max="100">
            </div>
        </div>
    ...
    <option value="Futebol" <?= (($_POST['modalidade'] ?? '') == 'Futebol') ? 'selected' : '' ?>>Futebol</option>
    <option value="Voleibol" <?= (($_POST['modalidade'] ?? '') == 'Voleibol') ? 'selected' : '' ?>>Voleibol</option>
    <option value="Basquetebol" <?= (($_POST['modalidade'] ?? '') == 'Basquetebol') ? 'selected' : '' ?>>Basquetebol</option>
    <option value="Sem modalidade" <?= (($_POST['modalidade'] ?? '') == 'Sem modalidade') ? 'selected' : '' ?>>Sem modalidade</option>
</select>
